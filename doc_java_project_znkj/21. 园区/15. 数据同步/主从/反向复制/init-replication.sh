...
~~~sql
-- 查看当前 repl_user 使用的认证插件
SELECT user, host, plugin FROM mysql.user WHERE user = 'repl_user';

-- 将 repl_user 的认证方式改为 mysql_native_password
...
# Slave1 向 Master 注册复制
SLAVE1_STATUS=$(mysql -h 192.168.0.8 -P 3305 -u root -prootpass -e "SHOW MASTER STATUS;" | grep mysql-bin)
...
SLAVE2_STATUS=$(mysql -h 192.168.0.8 -P 3305 -u root -prootpass -e "SHOW MASTER STATUS;" | grep mysql-bin)
